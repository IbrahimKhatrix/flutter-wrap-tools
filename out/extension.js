...
function expandSelectionToWidget() {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return null;
    ...
    return new vscode.Selection(doc.positionAt(doc.offsetAt(wordRange.start)), doc.positionAt(end));
}
...
function activate(context) {
    ...
    const wrapContainer = vscode.commands.registerCommand('flutterWrap.wrapWidget', () => {
        ...
        insertSnippet("${1:Widget}(\n" + getSpacer() + "child: ", "\n)$0", getSpacer());
    });
    const unwrap = vscode.commands.registerCommand('flutterWrap.unwrapWidget', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const doc = editor.document;
    ...
    const wrapStacked = vscode.commands.registerCommand('flutterWrap.wrapStacked', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const selection = editor.selection;
...
function hasTopLevelChildrenProperty(text) {
    let depth = 0;
    let inString = false;
    for (let i = 0; i < text.length; i++) {
        const c = text[i];
